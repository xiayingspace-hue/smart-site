...
4. **前端预检查**：调用 DC 配置查询接口，确认项目是否已配置 DC
   ...
   - 若**有 DC 配置**：继续下一步
5. 弹出确认对话框，文案说明"通过后进入外部审批阶段"
6. 点击 [Confirm]，后端执行内部审批通过逻辑（后端二次校验 DC 配置）
7. 成功：Todo 消失，Toast 提示 DC 已收到通知
...
flowchart TD
    ...
    D -- 通过 --> E[点击 Approve]
    D -- 驳回 --> F[点击 Reject]
    E --> E1{前端预检查：项目是否有 DC 配置?}
    E1 -- 无 DC --> E2[弹出无 DC 警告弹窗 F-004]
    E2 --> E3([流程终止，等待管理员配置 DC])
    E1 -- 有 DC --> G[弹出确认对话框]
    G --> H[点击 Confirm]
    H --> I{操作结果}
    I -- 成功 --> J[Todo 消失，Toast 提示 DC 已通知]
    I -- 失败（无 DC / 网络异常） --> K[Toast 报错，可重试]
...
| 异常场景 | 触发条件 | 系统响应 | 用户感知 |
|---------|---------|---------|---------|
| 项目无 DC 配置（前端预检） | 点击 [Approve] 时 DC 配置查询接口返回空列表 | 阻止打开确认对话框，弹出无 DC 警告弹窗（F-004） | 弹窗说明"当前项目无 DC，请联系管理员配置后再审批"，提供 [Got it] 关闭 |
...
### AC-007A-010：无 DC 配置时阻止内部审批通过

```
Given  项目当前无 DC 配置
When   内部审批人点击 [Approve]
Then   前端预检查后弹出无 DC 警告弹窗（F-004），不弹出确认对话框；
       弹窗文案提示联系管理员配置 DC；点击 [Got it] 后弹窗关闭，卡片状态不变
```

### AC-007A-011：无 DC 配置时后端兜底拒绝

```
Given  前端预检查被绕过，项目当前无 DC 配置
When   直接调用 POST /drawing/approve
Then   接口返回错误码 1003007012；若经由前端触发，loading 恢复，Toast 提示
       "No DC configured. Please ask admin to add a DC first."
```
