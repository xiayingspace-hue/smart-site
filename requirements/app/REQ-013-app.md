...
为提升现场工程师（SE）在移动端的工作效率和体验，需要为 SE 在 APP 端提供专属的"My Task"页面，便于其随时随地查看 CM 分配的工序任务并执行标记完成操作。
...
- 页面顶部为筛选/搜索区，支持按完成状态（未完成 / 已完成）、计划时间、优先级等筛选
- 下方为工序任务卡片列表，每张卡片展示：
  - 工序任务名称
  - 所属上级任务名称
  - 计划开始/结束时间
...
- [ ] SE 可查看工序任务详情，包括基本信息、计划时间、优先级、完成状态
- [ ] SE 可对"未完成"工序任务执行"标记完成"操作，操作前弹出锁定确认提示
- [ ] 标记完成后工序任务锁定，SE 无法自行撤销，上级任务进度自动更新
- [ ] SE 可对任务进行问题上报，提交后自动通知 CM
- [ ] SE 可收到工序任务相关消息通知
